// 레이아웃 목업 에디터 — 모델 사진 위에 상/하의 윤곽선을 얹고, 둘레의 컨트롤 포인트를
// 드래그해서 메쉬 와프(부분 늘리기/줄이기) 하거나, 옷 내부를 드래그해서 통째로 옮깁니다.
// [입히기]를 누르면 현재 캔버스 상태를 PNG로 캡처해 서버의 /api/synthesize로 보내
// 기존 garment-test.ts 파이프라인(GCP AI Studio 합성)을 그대로 실행합니다.
...
function targetCentroid(g) {
  return toWorld(g, centroidOf(g.basePoints), rotationPivot(g));
}
...
runBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // 캔버스/목업은 업로드한 모델 사진이 아니라 서버가 돌려준 더미(마네킹) 이미지 위에 옷을
    // 얹습니다. 업로드한 실제 모델 사진(modelDataUrl)은 그대로 state.modelDataUrl에 남아있다가
    // [입히기] 단계에서 최종 합성의 얼굴/포즈 소스로만 쓰입니다.
    ...
    setStatus('점을 드래그해서 조정한 뒤 [입히기]를 누르세요.');
  } catch (err) {
    console.error(err);
...
applyBtn.addEventListener('click', async () => {
  ...
  setStatus('현재 배치를 목업으로 저장하고 AI 합성 요청 중... (1~2분 정도 걸릴 수 있어요)');
  try {
    ...
    const res = await fetch('/api/synthesize', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: state.modelDataUrl,
        top: state.topSourceDataUrl,
        bottom: state.bottomSourceDataUrl,
    ...
    for (let i = 0; i < data.resultDataUrls.length; i++) {
      const item = document.createElement('div');
      item.className = 'resultItem';
      const img = document.createElement('img');
      img.src = data.resultDataUrls[i];
      img.addEventListener('click', () => openModal(img.src));
      const label = document.createElement('span');
      label.className = 'resultLabel';
      label.textContent = `결과 ${i + 1}`;
...
// ---- 초기 화면: 더미(마네킹)는 절대 안 바뀌므로 옷을 업로드/추출하기 전에도 바로 캔버스에
// 띄워둡니다. [참조이미지 만들기] 이후엔 서버가 돌려주는 dummyDataUrl로 다시 한번 갱신됩니다
// (캔버스 크기/스케일 재계산을 그 흐름과 동일하게 유지하기 위함일 뿐, 이미지 자체는 동일).
(async function initDummyCanvas() {
  try {
