...
// 모델 분석은 v2.3(/api/v23/analyze-model), 옷 종류 자동판정·핏·합성은 /api/v26/*를 씁니다.
...
async function autoGroupGarments(analysisModel) {
  ...
  try {
    ...
    const res = await fetch('/api/v26/group-garments', {
...
async function analyzeGarmentAt(slotKey, i, analysisModel, forcedCategory) {
  ...
  const res = await fetch('/api/v26/analyze-garment', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function analyzeWornAt(key, i, analysisModel) {
  ...
  const dataUrl = await toUploadDataUrl(file);
  const res = await fetch('/api/v26/classify-item', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
...
instructionsBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const res = await fetch('/api/v26/fitting-instructions', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
function runSynthShot(basePayload, engine) {
  ...
  (async () => {
    try {
      const res = await fetch('/api/v26/synthesize', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
